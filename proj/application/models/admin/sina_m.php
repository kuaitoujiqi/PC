...
class Sina_m extends CI_Model {
	...
	function create_p2p_hosting_borrowing_target($row = array())
	{	
		$post['out_bid_no'] = $row['id'];
		// $post['goods_id'] = $row['id'];
		$post['web_site_name'] = '快投机器';
		$post['bid_name'] = $row['title'];
		$post['bid_type'] = "OTHER";
		$post['bid_amount'] = $row['money'];//标的总金额
		$post['bid_year_rate'] = number_format($row['rate']*100,2,'.',''); //年华收益率
		$post['bid_duration'] = $row['day'] * 30;
		$post['term'] = date('YmdHis',strtotime('+'.$row['day'].' months',time())-(-172800));//还款期限
		$post['repay_type'] = "SCHEDULED_INTEREST_PAYMENTS_DUE";	 //按月付息
		$post['guarantee_method'] = "企业担保";//担保方式
		
		if($row['borrower_type'] == 3){			
			$post['borrower_info_list'] = $row['borrower_id']."~UID~".$row['money']."~".$row['use']."~".$row['phone'];//借款用户为个人 集合 Id^id_type^6000$
		}else{
			$post['borrower_info_list'] = $row['company']."~UID~".$row['money']."~".$row['use']."~".$row['phone'];//借款用户为企业 集合 Id^id_type^6000$			
		}
