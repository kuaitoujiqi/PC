...
class Usercenter extends Front_Controller {
	...
	function sina_show()
	{
		$userinfo = userinfo();
		$uid = $userinfo['id'];
		$return = $this->usercenter_m->show_member_infos_sina($uid);	
		var_dump($return);
	}
	...
	{   
		...
		// $query= $this->db->query("SELECT user_products.uid,user_products.projectid,bulk_standard.id,bulk_standard.title as alltitle,fr_order.id as allid,fr_order.type,fr_order.uid,fr_order.monkey,fr_order.dateline,user_products.monkey,red_paper.title,red_paper.monkey FROM (fr_order left join user_products on user_products.uid = fr_order.uid) left join red_paper on fr_order.monkey = red_paper.monkey left join bulk_standard on user_products.projectid = bulk_standard.id WHERE fr_order.type = 16 and user_products.monkey >= 5000 and fr_order.uid=$uid and red_paper.monkey>0 order by fr_order.id DESC limit $page,10");
		  // var_dump($query);
		  $queryall =  $this->db->query("SELECT fr_order.id as allid,fr_order.dateline as alltime,fr_order.monkey as allmoney,fr_order.productid as pid,bulk_standard.id as bid,bulk_standard.title as alltitle FROM fr_order LEFT JOIN bulk_standard on fr_order.productid = bulk_standard.id WHERE uid = $uid AND type = 16 AND remark = '发送红包'");
		  $query = $this->db->query("SELECT fr_order.id as allid,fr_order.dateline as alltime,fr_order.monkey as allmoney,fr_order.productid as pid,bulk_standard.id as bid,bulk_standard.title as alltitle FROM fr_order LEFT JOIN bulk_standard on fr_order.productid = bulk_standard.id WHERE uid = $uid AND type = 16 AND remark != '发送红包' order by fr_order.dateline desc limit $page,10");
	...
	{
		$this->load->library('form_validation');
		$this->form_validation->set_rules('oldpass', '旧密码', 'required|min_length[8]|max_length[16]|callback_check_password');
		$this->form_validation->set_rules('newpass', '新密码', 'required|min_length[8]|max_length[16]');
...
/*	function bind_bank($type = "add",$ajax = false)
	...
	function pact($id = false,$type = false)
	{
		...
		{ 
			...
			$this->load->view('pact/pactdown',$data);
		}else{
